...
dat = np.loadtxt("fig12_tau_cnt_o3_bremen.txt", skiprows=1)
wav_nm_o3 = dat[:, 0]
tau_o3 = dat[:, 1]

# NO2 Continuum: Bremen X-section data, US 1976 Atmopshere, Simpson integration over zkm
dat = np.loadtxt("fig12_tau_cnt_no2_bremen.txt", skiprows=1)
wav_nm_no2 = dat[:, 0]
tau_no2 = dat[:, 1]

# WV Continuum: MT_CKD v.4.3, US 1976 Atmopshere, Simpson integration over zkm
dat = np.loadtxt("fig12_tau_cnt_h2o_mtckd.txt", skiprows=1)
wav_nm_wvc = dat[:, 1]
tau_wvc = dat[:, 2]

# O2-O2 B-band: 671-704 nm - note two datasets
dat = np.loadtxt("fig12_tau_cia_o4_hitran_spiering_671_704nm.txt", skiprows=0)
